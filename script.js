...
// Buat helper kecil untuk tanggal dan waktu template
const tglIndo = () => {
  const d = new Date();
...
function renderTemplates(filter) {
  ...
  if (filtered.length === 0) {
    list.innerHTML = '<div style="padding:20px;text-align:center;color:var(--tx3);font-style:italic;">Template tidak ditemukan</div>';
    return;
  }
  list.innerHTML = filtered.map(
    (t, i) => {
      const realIndex = TEMPLATES.indexOf(t);
      return `<div class="tpl-item" onclick="useTemplate(${realIndex})">
